    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 25) {
                ...
                if showButton {
                    Button(action: { TechSound.play(.click); onComplete() }) {
                        HStack { Text("UNDERSTAND VECTORS"); Image(systemName: "arrow.up.forward.circle.fill") }
                            .font(.headline.bold()).foregroundColor(.xiloBlack).padding(.vertical, 16).padding(.horizontal, 30)
                            .background(Color.xiloCyan).cornerRadius(12)
                    }.transition(.scale)
                } else { Spacer().frame(height: 60) }
            }.padding()
        }
        .onAppear {
            Task {
                // Isso vai parar o som da intro e começar o 'meio' na hora
